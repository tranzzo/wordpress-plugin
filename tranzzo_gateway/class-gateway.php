  ...
  public function generate_form($order)
  {
    ...
    if (!empty($data_order)) {
      ...
      if ($this->enableFiscalization) {
        $apiService->setFReceiptDelivery();
      }
      ...
      if (count($data_order["line_items"]) > 0) {
        ...
        foreach ($data_order["line_items"] as $item) {
          ...
          if ($this->enableFiscalization) {
            ...
            $productFieldset["fiscalReceiptDelivery"] = "email";
          }
  ...
  public function checkTPResponse()
  {
    ...
    if ($apiService->validateSignature($data, $signature) && $order_id) {
      ...
      if (
        ...
        return;
        exit();
      } else if (/*$order->get_status() == "pending"*/ $data_response[ApiService::P_RES_STATUS] == ApiService::P_TRZ_ST_PENDING) {
